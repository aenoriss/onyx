...
class GeometryProcessor:
    ...
    @staticmethod
    def generate_views(n, pitch_offset=0, layout_mode='cube'):
        """
        ...
        if layout_mode == 'cube':
            ...
            for p_name, pitch in pitches:
                # COLMAP staggers yaw by 45° for positive pitch rows
                yaw_offset = 45.0 if pitch > 0 else 0.0
                names = stagger_names if pitch > 0 else yaw_names
                for y_name, base_yaw in zip(names, base_yaws):

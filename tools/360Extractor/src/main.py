...
def parse_arguments():
    ...
    parser.add_argument("--layout", type=str, choices=['ring', 'cube', 'fibonacci', 'colmap'], help="Camera layout mode (ring/cube/fibonacci/colmap, default: colmap)")
    ...
    parser.add_argument("--mode", type=str, choices=['360', 'standard'], default='360',
                        help="Extraction mode: '360' for equirectangular reprojection, 'standard' for raw frame extraction (default: 360)")
...
def run_cli(args):
    ...
    cam_count = args.camera_count if args.camera_count is not None else config.get('camera_count', 12)
    ...
    layout_mode = args.layout if args.layout is not None else config.get('layout_mode', 'colmap')

...
def training(
    ...
    for iteration in range(first_iter, opt.iterations + 1):   
        ...
        # Periodic opacity pruning - remove dead Gaussians early
        if iteration % 1000 == 0 and iteration > 2000:
            opacity = torch.sigmoid(gaussians._opacity).squeeze()
            low_opacity_mask = opacity < 0.05
            n_low = low_opacity_mask.sum().item()
            if n_low > 100:  # Only if significant
                print(f"[Opacity Prune] Removing {n_low} low-opacity Gaussians (iter {iteration})")
                gaussians.prune_points(low_opacity_mask)
